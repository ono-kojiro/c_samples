...
#if ENABLE_DEMUX
	int mode = 0x00;
	unsigned char g_buff_rd[TTY_RD_SZ] =  { 0 };
	int  g_buff_rd_len = 0;
#endif
    ...
    skip_proc_STI:
        ...
        if ( FD_ISSET(tty_fd, &rdset) ) {
...
#if ENABLE_DEMUX
                n = read(tty_fd,
					&g_buff_rd + g_buff_rd_len,
					sizeof(g_buff_rd) - g_buff_rd_len);
#else
            ...
            if (n == 0) {
				...
				g_buff_rd_len += n;
#if DEBUG
				fprintf(stderr, "g_buff_rd[%d] = {\r\n", g_buff_rd_len);
				for(i = 0; i < g_buff_rd_len; i++){
					fprintf(stderr, " 0x%02x", (unsigned char)g_buff_rd[i]);
				}
				fprintf(stderr, " }\r\n");
#endif
...
#if ENABLE_DEMUX
				if(opts.mode != 0xFF){
					available_len = g_buff_rd_len;

					for(i = 0; i < g_buff_rd_len; i++){
						if(i != g_buff_rd_len - 1 &&
							g_buff_rd[i] == 0xFF)
						{
							i++;
							//fprintf(stderr, "mode : 0x%02x -> 0x%02x\r\n",
							//	mode, g_buff_rd[i]);
							mode = g_buff_rd[i];
						
						}
						else if(i == g_buff_rd_len && 
							g_buff_rd[i] == 0xFF)
						{
							available_len = g_buff_rd_len - 1;
							break;
						}
						else if(opts.mode == mode){
        	            	bmp += do_map(bmp, opts.imap, g_buff_rd[i]);
						}
#if DEBUG
					...
					if(available_len == g_buff_rd_len){
						g_buff_rd_len = 0;
					}
					else{
						g_buff_rd[0]  = 0xFF;
